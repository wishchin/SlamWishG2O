...
Tracking::Tracking(ORBVocabulary* pVoc, 
    ...
    mState(NO_IMAGES_YET), mpORBVocabulary(pVoc),
		...
		mpMap(pMap), mnLastRelocFrameId(0), mbPublisherStopped(false), mbReseting(false), mbForceRelocalisation(false), mbMotionModel(false)
{
...
{
    // Load camera parameters from settings file
	this->m_DataSourceHard = DataSource;

	//this->mImageFileList.resize(imageFileList.size() );
	//for(int idx =0;idx <imageFileList.size();++idx ){
	//	mImageFileList[idx] = imageFileList[idx];
	//}
	this->mImageFileQue.resize(imageFileList.size() );
	for(int idx =0;idx <imageFileList.size();++idx )
		this->mImageFileQue[idx] = imageFileList[idx];
...
void Tracking::Run()
{
	...
	//读取视频帧
	while ( CAMERA == this->m_DataSourceHard )
	{
		Camera >> GrabOneImg;
		//Camera_R >> GrabRightImage;
		cv::imshow("Camera!", GrabOneImg);
		cvWaitKey(8);
		Tracking::GrabImage(GrabOneImg);
	}

	while(FILELOCAL == this->m_DataSourceHard && 0 !=this->mImageFileQue.size())
	{
		GrabOneImg = cv::imread(this->mImageFileQue.front().c_str() );

		std::string  imageFlag = this->mImageFileQue.front().c_str();
		//imageFlag = imageFlag.erase(imageFlag.find_last_of("/"),imageFlag.size() - imageFlag.find_last_of("/") );
		imageFlag = imageFlag.erase(0 ,imageFlag.find_last_of("/") );

		//cv::imshow(imageFlag.c_str(), GrabOneImg);//cv::imshow("FileListImage!", GrabOneImg);
		//cvWaitKey(8);

		this->mImageFileQue.pop_front();//去掉队列头！
		Tracking::GrabImage(GrabOneImg);
	}
	
}

void Tracking::GrabImage(const cv::Mat& img)
{
	//计时
	LARGE_INTEGER litmp;
	double qt,qf;
	double GrabTimestamp;
	...
	GrabTimestamp = qt/qf;     //计算timestamp，for win

    if(mState==WORKING || mState==LOST)
		mCurrentFrame = Frame(im,GrabTimestamp,mpORBextractor,mpORBVocabulary,mK,mDistCoef);
	else
		mCurrentFrame = Frame(im,GrabTimestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef);
...
void Tracking::CreateInitialMap(cv::Mat &Rcw, cv::Mat &tcw)
{
	...
	std::printf("New Map created with %d points",mpMap->MapPointsInMap()); //for win
    ...
    if(medianDepth<0 || pKFcur->TrackedMapPoints()<50)    //for win
    {
        //ROS_INFO("Wrong initialization, reseting...");
		printf("Wrong initialization, reseting...");  //for win
        Reset();
        return;
    }
...
int Tracking::getImageQueLength()
{
	return (this->mImageFileQue.size() );
}
