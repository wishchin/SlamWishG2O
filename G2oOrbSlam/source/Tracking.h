...
public:
    ...
    Frame mInitialFrame;
	cv::VideoCapture Camera;  //for win
	cv::VideoCapture Camera_R; //for win
	cv::Mat GrabOneImg;  //for win
	cv::Mat GrabRightImage;
...
protected:
    void GrabImage(const cv::Mat& img); //for win
    ...
    bool mbRGB;
	int m_DataSourceHard;

	//若使用本地图像进行测试，则复制原始文件名队列！
	std::vector<std::string>  mImageFileList;
	std::deque<std::string>   mImageFileQue;

    // Transfor broadcaster (for visualization in rviz)
    //tf::TransformBroadcaster mTfBr;
	public:
		int getImageQueLength();
};
