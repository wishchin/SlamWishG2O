...
MapPublisher::MapPublisher(Map* pMap):mpMap(pMap), mbCameraUpdated(false)
{
   ...
   // mReferencePoints.action  = visualization_msgs::Marker.ADD;
    ...
    //Configure Publisher
    //publisher = nh.advertise<ORB_SLAM::visualization_msgs::Marker>("ORB_SLAM/Map", 10);//ros::NodeHandle nh;
...
{
	bool isCamUpdatedL = this->isCamUpdated();
	//boost::mutex::scoped_lock lock(mMutexCamera);
 //   //return mbCameraUpdated;
	isCamUpdatedL = this->mbCameraUpdated;

    if(isCamUpdatedL )
    {
       cv::Mat Tcw = GetCurrentCameraPose();
       this->PublishCurrentCamera(Tcw);
       ResetCamFlag();
    }

	bool isMapUpdatedL = mpMap->isMapUpdated();
	isMapUpdatedL = mpMap->pmbMapUpdated;
    ...
    if(isMapUpdatedL  )
    {
        ...
        this->PublishMapPoints(vMapPoints, vRefMapPoints);   
        this->PublishKeyFrames(vKeyFrames);
...
void MapPublisher::PublishKeyFrames(const vector<KeyFrame*> &vpKFs)
{
    ...
    for(size_t i=0, iend=vpKFs.size() ;i<iend; i++)
    {
		...
		pcl::PointXYZRGB   msgs_o,msgs_p1, msgs_p2, msgs_p3, msgs_p4;
...
{
    boost::mutex::scoped_lock lock(mMutexCamera);
    this->mCameraPose = Tcw.clone();
    this->mbCameraUpdated = true;
}

cv::Mat MapPublisher::GetCurrentCameraPose()
{
    boost::mutex::scoped_lock lock(mMutexCamera);
    return this->mCameraPose.clone();
}

bool MapPublisher::isCamUpdated()
{
    boost::mutex::scoped_lock lock(mMutexCamera);
    return this->mbCameraUpdated;
}

void MapPublisher::ResetCamFlag()
{
    boost::mutex::scoped_lock lock(mMutexCamera);
    this->mbCameraUpdated = false;
}
