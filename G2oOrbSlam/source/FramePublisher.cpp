...
cv::Mat FramePublisher::DrawFrame()
{
	...
	cv::Mat im3C;
	if(1 == im.channels()){//<3
		cvtColor(im,im3C,CV_GRAY2BGR);//cvtColor(im,im,CV_GRAY2BGR);
	}
	else
	{
		im.copyTo(im3C );
    ...
    if(state==Tracking::INITIALIZING) //INITIALIZING
    {
        for(unsigned int i=0; i<vMatches.size(); i++)
        {
            if(vMatches[i]>=0)
            {
                cv::line(im3C,vIniKeys[i].pt,//cv::line(im,vIniKeys[i].pt,
    ...
    else if(state==Tracking::WORKING) //TRACKING
    {
        ...
        for(unsigned int i=0;i<vMatchedMapPoints.size();i++)
        {
            if(vMatchedMapPoints[i] || mvbOutliers[i])
            {
                ...
                if(!mvbOutliers[i])
                {
                    cv::rectangle(im3C,pt1,pt2,cv::Scalar(0,255,0));//cv::rectangle(im,pt1,pt2,cv::Scalar(0,255,0));
                    cv::circle(im3C,vCurrentKeys[i].pt,2,cv::Scalar(0,255,0),-1);//cv::circle(im,vCurrentKeys[i].pt,2,cv::Scalar(0,255,0),-1);
                    mnTracked++;
                }
            }
        }
    ...
    cv::Mat imWithInfo;
    DrawTextInfo(im3C,state, imWithInfo);// DrawTextInfo(im,state, imWithInfo);
	//im3C.copyTo(imWithInfo );
    return imWithInfo;
}
...
{
    boost::mutex::scoped_lock lock(mMutex);
	CurrentRightImage = pTracker->GrabRightImage.clone();
	CurrentLeftImage = pTracker->GrabOneImg.clone();
...
vector<MapPoint*> FramePublisher::GetMatchedMapPoints()
{
	boost::mutex::scoped_lock lock(mMutex);
	return mvpMatchedMapPoints;
}

vector<bool> FramePublisher::GetOutliers()
{
	boost::mutex::scoped_lock lock(mMutex);
	return mvbOutliers;
}

vector<cv::KeyPoint> FramePublisher::GetCurrentKeys()
{
	boost::mutex::scoped_lock lock(mMutex);
	return mvCurrentKeys;
}

cv::Mat FramePublisher::GetCurrentImage()
{
	boost::mutex::scoped_lock lock(mMutex);
	return mIm;
}
